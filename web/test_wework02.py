...
class TestWework:
    def setup(self):
        # options=Options()
        # options.debugger_address="127.0.0.1:9222"                         #============ http protocal cannt be added
        # self.driver=webdriver.Chrome(options=options)                     #============ after get the cookies, then restart a chrome.exe
    ...
    def test_cookie(self):
        ...
        # cookies=self.driver.get_cookies()        #========= if cookies exoires, you need to acuire it again by yourself
        ...
        localdb = shelve.open("cookies")            #====== create one file if not exists  it is a local database
        # localdb['cookies']=cookies
        cookies=localdb['cookies']
        print(cookies)
        for cookie in cookies:
            if 'expiry' in cookie.keys():
                cookie.pop('expiry')
            self.driver.add_cookie(cookie)     #======  cookies injection  python usage
        self.driver.refresh()
